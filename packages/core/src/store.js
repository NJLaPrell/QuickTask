...
export function getTaskTemplate(store, taskName) {
    ...
    try {
        ...
        throw new Error(`Failed to read task template ${filename}: ${error instanceof Error ? error.message : 'unknown error'}`);
    }
    try {
        ...
        throw new Error(`Template ${filename} is corrupted and was moved to ${path.basename(backupPath)}: ${error instanceof Error ? error.message : 'unknown error'}`);
    }
}
export function saveTaskTemplate(store, template) {
    ...
    try {
        ...
        throw new Error(`Failed to save task template ${filename}: ${error instanceof Error ? error.message : 'unknown error'}`);
    }
    finally {
        releaseLock();
    }
