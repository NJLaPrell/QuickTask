...
test('normalizes implicit command to /qt create form', () => {
  assert.equal(normalizeOpenClawQtInput('summarize concise bullets'), '/qt summarize concise bullets')
})

test('preserves explicit /qt command', () => {
  assert.equal(normalizeOpenClawQtInput('/qt improve summarize add actions'), '/qt improve summarize add actions')
})
