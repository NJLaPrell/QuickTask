...
**Resolved here:** `qt:sandbox` from repo root — workspace devDependency on `@quicktask/core`, CI runs `pnpm qt:sandbox -- "/qt help all"` (see `CONTRIBUTORS.md`, T138).
...
- `docs/qt-command-result-contract.md` — behavior contracts when core/parser changes (includes **Verbose and debug mode (spec)** for UF-005 / UF-017).
